...
public class CallAndRespond_Dynamic extends MaxObject{
    ...
    float[][] pulsePattern = new float[][] {{0.5f, 62}, {-1, -1}, {1.f, 50}, {-1, -1}, {0.5f, 62}};
    ArrayList<float[][]> learned_motifs = new ArrayList<>();
    ArrayList<float[][]> coded_motifs = new ArrayList<>();
    ArrayList<float[]> rhythms = new ArrayList<>();
    ...
    int motif_counter = 0;
    final int num_coded_motifs;
    final int num_learn_motifs;
    final int total_motifs;
    int num_musicians = 4;
    ...
    public CallAndRespond_Dynamic(int num_learn_motifs, int num_coded_motifs, int num_musicians){
        this.num_learn_motifs = num_learn_motifs;
        this.num_coded_motifs = num_coded_motifs;
        this.total_motifs = num_learn_motifs + num_coded_motifs;
        this.num_musicians = num_musicians;
        ...
        float[][][] hipHops = new float[][][]{hipHopMotif1, hipHopMotif2, hipHopMotif3, hipHopMotif4, hipHopMotif1};
        ...
        float[][] hipHopRhythms = new float[][] {rhythm1, rhythm2, rhythm3, rhythm4, rhythm1};

        for(int i = 0; i < num_coded_motifs; i++){
            coded_motifs.add(hipHops[i % hipHops.length]);
        }

        for(int i = 0; i < num_learn_motifs; i++){
            learned_motifs.add(hipHops[i % hipHops.length]);
        }

        for(int i = 0; i < num_learn_motifs + num_coded_motifs; i++){
            rhythms.add(hipHopRhythms[i % hipHopRhythms.length]);
        }
    ...
    public void playBack(){
        ...
        if(true) {
            ...
            if (new_motif != null) {
                float[][] pattern_added = new_motif;
                float[] pattern = pulsePattern[motif_counter % pulsePattern.length];
                if(pattern[0] > 0 && pattern[1] > 0) {
                    pattern_added = MotifLearner.addPulse(new_motif, pattern[0], pattern[1]);
                }
                learned_motifs.set(motif_counter, pattern_added);

                motif_collection.add(new_motif);
                motif_counter = (motif_counter + 1) % num_learn_motifs;
    ...
    public void scheduleRhythms(long waitTime) {
        ...
        rhythm = rhythms.get(currentSection % rhythms.size());
            ...
            if(learnedMotif == null) {
                switch (currentSection) {
                    case -1:
                        ...
                        break;
                    default:
                        if(currentSection >= total_motifs){
                            endNodCount++;
                            waitTime = (int) headNodInterval;
                            if (endNodCount >= 8) {
                                currentSection = 6;
                            ...
                            float[][] currentMotif;
                            if(currentSection < num_learn_motifs){
                                currentMotif = learned_motifs.get(currentSection);
                                rnd_len = randomizer == 1 ? randomMotif[0].length : 0;
                                note = randomizer == 1 ? randomMotif[0][hiphopIndex % rnd_len] : currentMotif[0][hiphopIndex];
                            } else{
                                currentMotif = coded_motifs.get(currentSection - num_learn_motifs);
                                note = currentMotif[0][hiphopIndex];
                            }
                            outlet(0, note);
                            waitTime = (int) (headNodInterval * currentMotif[1][hiphopIndex]);
                            scheduleRhythms(waitTime);
                            hiphopIndex++;
                            if (hiphopIndex >= currentMotif[0].length) {
                                checkSection();
                            }
