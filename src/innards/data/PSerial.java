...
// Adapted by Siggi from the Processing libraries which use rxtx for native libraries for all platforms
...
public class PSerial implements SerialPortEventListener {

	static {
		//ResourceLocator.loadLibrary("rxtxSerial");
	}
	...
	public PSerial() throws IOException {
		this(dname, drate, dparity, ddatabits, dstopbits);
	}

	public PSerial(int irate) throws IOException {
		this(dname, irate, dparity, ddatabits, dstopbits);
	}

	public PSerial(String iname, int irate) throws IOException {
		this(iname, irate, dparity, ddatabits, dstopbits);
	}

	public PSerial(String iname) throws IOException {
		this(iname, drate, dparity, ddatabits, dstopbits);
	}

	public PSerial(String iname, int irate,
			char iparity, int idatabits, float istopbits) throws IOException {
...
//			e.printStackTrace();
			...
			input = null;
			output = null;
			throw new IOException("Couldn't open port "+iname+" at baud rate "+irate);
		}
		
		if( port == null ){
			throw new IOException("Couldn't find port with name "+iname);
		}
...
//		}
	 ...
	 */
	public void dispose() {
		// Siggi: I am commenting this out because it causes errors
		// It seems that rxtx binaries don't like us closing the port from the java side
...
//		port = null;
	...
	public void setSpeed(int newRate) throws UnsupportedCommOperationException  {
		port.setSerialPortParams(newRate, databits, stopbits, parity);
		rate = newRate;
	}
...
//						}
	...
	/**
	 * Returns the byte that is at position pos in buffer (pos 0 is the freshest byte)
	 * this method doesn't change the buffer in any way, only looks at a byte
	 * returns -1 if error
	 */
	public int peekAtByte(int pos) {
		if (bufferIndex + pos >= bufferLast) return -1;

		int outgoing;
		synchronized (buffer) {
			outgoing = ((int)buffer[bufferIndex + pos] & 0xff);
		}
		return outgoing;
	}
	...
	public int readByteReturnInt() {
		byte outgoing = readByte();
		return ((int)outgoing & 0xff);
	}
